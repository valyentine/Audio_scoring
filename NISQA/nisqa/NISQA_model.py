...
class nisqaModel(object):
    ...
    def predict(self):
        ...
        if self.args['output_dir']:
            ...
            if self.args['mode']=="predict_dir":
                dir_path=self.args['data_dir']
                folder_name = os.path.basename(os.path.normpath(dir_path))
                self.ds_val.df.to_csv(
                    os.path.join(self.args['output_dir'], f'{folder_name}_NISQA_results.csv'),
                index=False)
            else:
                file_path = self.args['deg']
                file_name = os.path.basename(os.path.normpath(file_path))
                self.ds_val.df.to_csv(
                    os.path.join(self.args['output_dir'], f'{file_name}_NISQA_results.csv'),
                index=False)
